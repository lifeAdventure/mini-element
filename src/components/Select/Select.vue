<template>
  ...
  >
    ...
    >
      ...
      <template #content>
        <!-- loading状态 -->
        <div class="mini-select__loading" v-if="states.loading">
          <Icon icon="spinner" spin />
        </div>

        <!-- 没有搜索结果 -->
        <div v-else-if="filterable && fillterOptions.length === 0" class="mini-select__nodata">
          <span>No matching results</span>
        </div>

        <ul class="mini-select__menu" v-else>
          <template v-for="(item, index) in fillterOptions" :key="index">
            <li
              class="mini-select__menu-item"
              :class="{
